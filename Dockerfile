...
HEALTHCHECK --interval=30s --timeout=5s --start-period=20s --retries=3 \
  CMD python -c "import os,urllib.request,sys; p=os.environ.get('PORT') or ('7860' if os.environ.get('SPACE_ID') else '8000'); sys.exit(0 if urllib.request.urlopen('http://127.0.0.1:'+p+'/api/health').status == 200 else 1)"
